...
6. En el modal de resultados, pulsar **Ver detalle de cada prueba** para ver el resultado prueba a prueba.
7. Pulsar **Generar JSON TestSubmit** para abrir el contenido del fichero `_TestSubmit.js` generado automáticamente desde `nombreentidad_pruebas`, con un botón para copiarlo al portapapeles.

> Los modales (tanto el de resultados de test como el de errores) se cierran con la **"X"**, con la tecla **Escape** y al **pulsar fuera** (sobre el fondo oscuro).
>
...
- **Ventanas de test y errores: modales dentro de la página**: el enunciado pide que la información de los test se muestre en "ventanas" con scroll horizontal y vertical y que los mensajes de error sean modales. Todo se resuelve con **modales dentro de la propia página** (no se abren ventanas del navegador): el modal de resultados (`#result_overlay`, clase `modal-large`) muestra el resumen y el detalle de los test, con **scroll horizontal y vertical definido en `IU.css`** (`.modal-large .modal-body { overflow-x: auto; overflow-y: auto }`); el modal de error (`#modal_overlay`) muestra los mensajes de error. Ambos se cierran con la **"X"**, con **Escape** y al **pulsar fuera** (sobre el fondo oscuro). Todas las reglas de estilo, incluido el scroll, viven en `css/IU.css`.
...
- **SEARCH: campo vacío opcional en submit, pero `format` exige valor**: en una búsqueda, dejar un campo vacío significa "no filtrar por él", así que `CheckSubmit` **omite** los campos SEARCH vacíos (no los valida). Ahora bien, los `format` de SEARCH **no aceptan cadena vacía** en ninguna de las tres entidades (criterio homogéneo): si una prueba de campo evalúa el formato de un valor en SEARCH, ese valor debe existir. Las dos capas son coherentes: el vacío se gestiona omitiéndolo en submit, no haciendo que el patrón lo dé por válido.
