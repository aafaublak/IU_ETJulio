 ...
 * Si no existe el fichero _TestSubmit.js genera las pruebas
 * automaticamente a partir de las pruebas de atributo.
 */
class TestSubmit {
    ...
    renderSummary() {
        ...
        var btnExport = document.getElementById("btn_export_json_submit");
        if (btnExport) {
            var self = this;
     ...
     * de submit: resumen por accion y detalle prueba a prueba con
     * resultado esperado, obtenido y si fue correcta o no.
     *
     * @returns {void}
     */
    exportJSON() {
        var data = {
            ...
            autoGeneradas: this.autoGenerated,
            resumen: {
                total: this.results.total,
                correctas: this.results.totalCorrect,
                incorrectas: this.results.totalIncorrect,
                porAccion: this.results.byAction
            },
            detalle: this.results.allDetails.map(function(t) {
                var d = t.data || [];
                return {
                    numPrueba: t.index + 1,
                    accion: d[1] || "",
                    descripcion: d[3] || "",
                    resultadoEsperado: t.resultExpected,
                    resultadoObtenido: t.resultObtained,
                    correcta: t.passed
                };
            })
        };
        ...
        a.href = url;
        a.download = "tests_submit_" + this.entityName + ".json";
    ...
    buildSummaryHTML(results) {
        ...
        html += '<button class="btn" id="btn_export_json_submit">Exportar resultados JSON</button>';
        html += '</div>';
        html += '</div>';
        return html;
    }
